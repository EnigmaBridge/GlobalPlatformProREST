...
apikey = "93446a32e2fefda398a46a4349172351"
dataset = "test"
...
if __name__ == "__main__":
  ...
  if _MODE == "PUSH":
    ...
    data = '{"fields":{"value":{"type":"number","name":"DanValue", "optional":false}, "timestamp":{"type":"datetime","name":"Time" }}, "unique_by":["timestamp"]  }'
    payload = json.loads(data)
    print payload
    session = requests.Session()
    session.headers = ('content-type', 'application/json')
    req = requests.Request('PUT', 'https://api.geckoboard.com/datasets/' + dataset, auth=HTTPBasicAuth(apikey,''), data=data)
    prepped = req.prepare()

    resp = session.send(prepped, timeout=2)
    ...
    while True:
      try:
        ...
        tepoch = datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ')
        data = '{"value":' + str(testV) + ',"timestamp":"' + str(tepoch) + '"}'
        data = '{"data":[' + data + ']}'
        ...
        session = requests.Session()
        session.headers = ('content-type', 'application/json')
        req = requests.Request('POST', 'https://api.geckoboard.com/datasets/' + dataset + '/data', data=data, auth=HTTPBasicAuth(apikey,''))
        prepped = req.prepare()
